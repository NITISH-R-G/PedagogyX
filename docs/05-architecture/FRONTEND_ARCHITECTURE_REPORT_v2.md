...
- **Component Structure:** The Next.js app router application uses a strict component hierarchy. UI components are stateless and presentational (using Radix UI and Tailwind CSS), while feature components encapsulate domain logic and state.
- **State Management:** Server-side state and initial data fetching are handled by Next.js React Server Components with aggressive caching. Client-side interactive state is managed via lightweight context or Zustand.
...
- **Design System Strategy:** Tailwind CSS v4 is used with CSS-based configuration (PostCSS and `@import "tailwindcss";`) to define a single source of truth for design tokens. The system enforces strict visual mapping for statuses.
