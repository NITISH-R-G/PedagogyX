...
| **Product goal** | Capture lessons for **teacher pedagogy monitoring and assessment** (admin sees **per-teacher** lesson quality, not per-student league tables on the client) |
| **Client role** | Thin capture + upload only; pedagogy scoring on **central** GPU path |

Pilot sites may log make/model for the compatibility matrix; see [INDIA_PILOT_DEVICE_REFERENCE.md](../01-phase0-founder-interrogation/INDIA_PILOT_DEVICE_REFERENCE.md) for **example** brands only.
...
Any Android panel or Windows smartboard **in scope** if it passes the profile for its tier (automated soak test + manual pilot checklist).
...
| **Android B** | 2–3 GB RAM, older SoC | Screen + mic only | Document as unsupported for cam |
...
**Examples** (non-exhaustive): ViewSonic, BenQ, Samsung Flip, LG CreateBoard, Teachmint, Promark, generic GeM panels — see device reference.
