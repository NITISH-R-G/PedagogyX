...
- Will the system offer explainable AI features so teachers can understand _why_ they received a specific pedagogical score?
...
- **Assumed Architecture:** Cloud-native web platform with asynchronous video upload and commenting.
- **Inferred Pipeline:** User uploads video -> cloud storage -> manual/AI tagging -> asynchronous coaching feedback.
- **Probable Stack:** Likely AWS, React, Node.js/Python, standard RDBMS.
...
- **Business Model:** B2B SaaS for schools and districts, subscription-based.
- **Scalability Constraints:** High storage costs due to video hosting, compute costs relatively low (mostly standard web traffic).
- **Likely Infra Costs:** Moderate, dominated by S3/EBS storage.
- **UX Observations:** Focused on reflective practice, timeline-based commenting is intuitive but manual.
- **Differentiators:** AI Coach feature introduces automated feedback, but still heavily reliant on user-initiated workflows.
- **Missing Features:** Real-time edge capture (e.g., smart glasses), zero-click ingestion, deep multimodal sentiment analysis.
- **Opportunities for Disruption:** PedagogyX's automated, ambient capture via Ray-Bans removes the friction of manual recording and provides continuous, invisible intelligence.

### Vosaic

- **Type:** Video-Based Reflection and Coaching
- **Assumed Architecture:** Cloud platform emphasizing custom rubrics and time-stamped video analysis.
- **Inferred Pipeline:** Video ingestion -> transcoding -> manual coding against rubrics -> reporting.
- **Probable Stack:** Cloud-hosted, likely monolithic backend with a robust video player frontend.
...
- **Business Model:** SaaS subscription, tiered by users/storage.
- **Scalability Constraints:** Scales well for storage, but manual workflows limit the volume of video actually analyzed.
- **Likely Infra Costs:** Moderate, primarily storage and transcoding.
- **UX Observations:** Clinical and professional, heavy emphasis on forms and rubrics.
- **Differentiators:** Flexibility in rubric creation.
- **Missing Features:** Autonomous pedagogical scoring, multimodal engagement heatmaps, ambient hardware integration.
- **Opportunities for Disruption:** Replacing manual coding with autonomous multimodal event detection (speech + vision) directly from the edge.

### IRIS Connect

- **Type:** Professional Development Platform
- **Assumed Architecture:** Hardware-software combo (Discovery Kit cameras + cloud platform).
- **Inferred Pipeline:** Proprietary hardware capture -> secure cloud upload -> reflection and sharing.
- **Probable Stack:** Custom IoT hardware, secure cloud backend, robust permissioning system.
...
- **Scalability Constraints:** Hardware logistics constrain rapid scaling compared to software-only or consumer-hardware solutions.
- **Likely Infra Costs:** High upfront hardware costs, moderate cloud costs.
- **UX Observations:** Secure and structured, emphasizes trust and permissioned sharing.
- **Differentiators:** Integrated hardware ecosystem.
- **Missing Features:** Discrete form factor (glasses), advanced real-time AI analytics (relies heavily on peer review).
- **Opportunities for Disruption:** Using consumer smart glasses (Meta Ray-Ban) eliminates the need for expensive, bulky proprietary camera kits while offering better POV data.

### AI Sokrates

- **Type:** AI Teaching Assistant / Analytics
- **Assumed Architecture:** Cloud-based AI analytics engine applied to lesson plans and recordings.
- **Inferred Pipeline:** Data ingestion -> NLP/audio analysis -> reporting dashboards.
- **Probable Stack:** Modern AI stack, Python/PyTorch backend, vector embeddings.
...
- **Business Model:** SaaS.
- **Scalability Constraints:** High inference costs depending on model size.
- **Likely Infra Costs:** High GPU compute costs.
- **UX Observations:** Data-heavy dashboards, can be overwhelming if insights aren't actionable.
- **Differentiators:** Pure AI play.
- **Missing Features:** Seamless hardware integration, edge processing.
- **Opportunities for Disruption:** Combining elite AI analytics with frictionless edge capture (Ray-Bans) provides superior data quality and user experience.

### Chinese Smart Classroom Systems (Various)

- **Type:** Surveillance and Analytics
- **Assumed Architecture:** Heavy edge compute (on-premise servers) + central state/district cloud.
- **Inferred Pipeline:** Multi-camera IP streams -> edge AI processing (facial recognition, posture) -> real-time dashboard -> central aggregation.
- **Probable Stack:** Hikvision/Dahua hardware, local GPU clusters, proprietary computer vision models.
...
- **Business Model:** B2G, massive infrastructure contracts.
- **Scalability Constraints:** Requires immense local compute power.
- **Likely Infra Costs:** Extremely high hardware and installation costs per classroom.
- **UX Observations:** Panopticon-style dashboards, punitive rather than coaching-focused.
- **Differentiators:** Total surveillance capability.
- **Missing Features:** Privacy, teacher agency, constructive pedagogical frameworks.
- **Opportunities for Disruption:** PedagogyX provides deep insights without the dystopian surveillance, using privacy-preserving edge models and focusing on teacher empowerment.

### Zoom AI Companion / Teams Analytics

- **Type:** General Meeting Intelligence
- **Assumed Architecture:** Massive global cloud infrastructure, deep integration with video conferencing clients.
- **Inferred Pipeline:** Live WebRTC stream -> cloud transcription/summarization -> post-meeting artifacts.
- **Probable Stack:** C++, WebRTC, enormous proprietary ML clusters.
...
- **Business Model:** Bundled enterprise SaaS.
- **Scalability Constraints:** Global hyper-scale.
- **Likely Infra Costs:** Massive, but subsidized by the core business.
- **UX Observations:** Seamlessly integrated into the meeting workflow.
- **Differentiators:** Scale and integration.
- **Missing Features:** Physical classroom capture, specific pedagogical rubrics, student engagement tracking (beyond simple attention metrics).
- **Opportunities for Disruption:** PedagogyX is purpose-built for the complexities of the physical and hybrid classroom, not just online meetings.
...
### Multimodal Transformers for Classroom Activity Recognition
...
### Speech Emotion Recognition in Noisy Educational Environments
...
### Pedagogical Action Detection via Long-Context Video Understanding
...
### Privacy-Preserving Federated Learning for Student Engagement Modeling
...
### Automated Scoring of Teacher Discourse Quality using LLMs
...
- **Go:** Excellent for concurrent ingestion of live streams (e.g., edge buffers). Low memory footprint. _Decision:_ Use for high-throughput edge/LAN ingestion APIs.
- **Python (FastAPI):** Unmatched for ML integration and orchestration. Slower concurrency, but necessary for the AI backend. _Decision:_ Core AI API and worker orchestration.
- **Rust:** Highest performance, steep learning curve. _Decision:_ Hold for critical performance bottlenecks (e.g., custom video transcoders), otherwise stick to Go/Python.
- **Node.js:** Good ecosystem, but worse CPU-bound performance than Go and worse ML ecosystem than Python. _Decision:_ Avoid for backend, restrict to frontend tooling.

### AI/ML Frameworks

- **PyTorch:** The absolute standard for research and modern models. _Decision:_ Primary framework for model development and fine-tuning.
- **TensorRT / ONNX:** Essential for production inference. _Decision:_ Export PyTorch models to TensorRT for deployment on the RTX 5070 dev GPUs and future cloud deployments to maximize throughput.
- **JAX:** Great for TPUs, but we are standardizing on NVIDIA GPUs. _Decision:_ Pass.

### Video Pipelines

- **FFmpeg:** The workhorse. Reliable but hard to programmatically orchestrate at scale. _Decision:_ Use for basic extraction and offline jobs.
- **GStreamer:** Powerful, complex pipeline construction. _Decision:_ Use for real-time edge processing if needed, but overhead might be high for simple phone apps.
- **WebRTC:** Low latency, built for streaming. _Decision:_ Evaluate if live, real-time dashboarding is required; otherwise, robust chunked HTTP/WebSocket uploads (DAT) from the phone are simpler for offline-tolerant capture.

### Databases

- **Postgres:** The source of truth for structured data (users, metadata, session state). _Decision:_ Primary relational store.
- **Redis:** Message brokering and caching. _Decision:_ Core to the Python Celery/RQ worker queues.
- **MinIO (S3):** Object storage for video and audio chunks. _Decision:_ Required for scalable, cost-effective blob storage.
- **Qdrant / Milvus:** Vector databases for multimodal embeddings. _Decision:_ Qdrant offers a good balance of Rust-based performance and simplicity for our phase.

### Frontend

- **React / Next.js (App Router):** Industry standard, excellent developer experience, easy to hire. _Decision:_ Primary web stack (already established in `services/web`).
- **Tauri / Electron:** Not needed yet, as the primary capture device is the Ray-Ban + Android phone.

### Infrastructure & Cloud

- **Kubernetes:** The standard for scalable microservices. _Decision:_ Use for cloud deployments to manage the worker fleets.
- **Docker Compose:** _Decision:_ Maintain for the local dev/MVP stack (`infra/compose.dev.yml`) to ensure rapid developer onboarding.
- **Self-Hosted GPUs (RTX 5070):** Essential for keeping Phase 1 / Pilot inference costs manageable before committing to massive AWS/GCP bills. _Decision:_ Hybrid architecture; edge/local capture, central OSS backend on self-managed or bare-metal GPU instances initially.
