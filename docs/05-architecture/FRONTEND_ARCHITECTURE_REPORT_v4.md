...
- **Simplifications:** Migrate any remaining raw inline styles to Tailwind CSS utility classes. (e.g. `style={{ width: \`\${Math.min(100, mA.coverage_pct)}%\` }}` -> inline dynamic style ok for width but other layout styles should be classes)
