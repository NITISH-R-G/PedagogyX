...
**Context:** PedagogyX Phase 0 + MVP boilerplate, updated with Meta Ray-Ban (DAT) primary client path.
...
- **Scaling Strategy:** Connection pooling (PgBouncer) to prevent worker-side connection starvation. Future read-replicas for dashboard queries.
