...
- **Validity:** Weak to Moderate. Studies (e.g., Kaur et al., 2016 DAiSEE dataset) show that posture, gaze, and facial expressions correlate with *behavioral* engagement, but not necessarily *cognitive* engagement.
...
- **Recommendation:** If a composite index is required for the "supervision" mode, it must be framed as an *indicative proxy* rather than a definitive evaluation. The UI should emphasize the sub-metrics (e.g., "Wait Time", "Questioning Rate") over the final rolled-up number.
