...
class TestComputeTalkRatio(unittest.TestCase):
    ...
    @patch("worker.metrics_main.psycopg2.connect")
    def test_compute_talk_ratio_null_column(self, mock_connect):
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_connect.return_value.__enter__.return_value = mock_conn
        mock_conn.cursor.return_value.__enter__.return_value = mock_cursor
    ...
    @patch("worker.metrics_main.psycopg2.connect")
    def test_compute_talk_ratio_missing_keys(self, mock_connect):
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_connect.return_value.__enter__.return_value = mock_conn
        mock_conn.cursor.return_value.__enter__.return_value = mock_cursor
        ...
        # index 1: max(0, 0-5)=0 (student)
        # index 2: max(0, 10-0)=10 (teacher)
        # total_dur = 10, teacher_dur = 10 -> ratio = 1.0
        self.assertEqual(teacher, 1.0)
        self.assertEqual(student, 0.0)
        self.assertEqual(confidence, "preview_heuristic")

    @patch("worker.metrics_main.psycopg2.connect")
    def test_compute_talk_ratio_negative_duration(self, mock_connect):
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_connect.return_value.__enter__.return_value = mock_conn
        mock_conn.cursor.return_value.__enter__.return_value = mock_cursor
    ...
    @patch("worker.metrics_main.psycopg2.connect")
    def test_compute_talk_ratio_string_numbers(self, mock_connect):
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_connect.return_value.__enter__.return_value = mock_conn
        mock_conn.cursor.return_value.__enter__.return_value = mock_cursor
