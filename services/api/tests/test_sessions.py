...
def test_upload_chunk_success(mock_insert_chunk, mock_put_chunk, mock_get_session):
    ...
    assert mock_put_chunk.call_count == 1
    args, _ = mock_put_chunk.call_args
    assert args[0] == mock_id
    assert args[1] == 1
    # File is closed after request, we just assert it's passed
    assert hasattr(args[2], "read")
    assert args[3] == "text/plain"
