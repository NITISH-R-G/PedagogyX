...
def test_create_dat_session(auth_client):
    mock_id = uuid.uuid4()
    mock_row = {
        "id": mock_id,
        "school_id": "school_1",
...
def test_get_dat_session_not_found(auth_client):
    dat_session_id = uuid.uuid4()
    with patch("app.dat_routes.dat_db.get_dat_session") as mock_get:
        mock_get.return_value = None

        response = auth_client.get(f"/v1/dat-sessions/{dat_session_id}")
        assert response.status_code == 404
        assert response.json()["detail"] == "dat session not found"


def test_get_dat_session_success(auth_client):
    dat_session_id = uuid.uuid4()
    mock_row = {
        "id": dat_session_id,
        "school_id": "school_1",
...
def test_post_lifecycle_session(auth_client):
    dat_session_id = uuid.uuid4()
    mock_row = {
        "id": dat_session_id,
        "school_id": "school_1",
...
def test_post_lifecycle_stream_with_pedagogy_link(auth_client):
    dat_session_id = uuid.uuid4()
    mock_row = {
        "id": dat_session_id,
        "school_id": "school_1",
...
def test_start_dat_session(auth_client):
    dat_session_id = uuid.uuid4()
    mock_row = {
        "id": dat_session_id,
        "school_id": "school_1",
    ...
    with patch("app.dat_routes.dat_db.transition_session_state") as mock_transition:
        ...
        response = auth_client.post(f"/v1/dat-sessions/{dat_session_id}/start")
        assert response.status_code == 200
        assert response.json()["state"] == "STARTED"


def test_start_stream(auth_client):
    dat_session_id = uuid.uuid4()
    mock_row = {
        "id": dat_session_id,
        "school_id": "school_1",
    ...
    with (
        ...
        response = auth_client.post(f"/v1/dat-sessions/{dat_session_id}/stream/start")
        assert response.status_code == 200
